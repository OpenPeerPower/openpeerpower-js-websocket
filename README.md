# :aerial_tramway: JavaScript websocket client for Open Peer Power

This is a websocket client written in JavaScript that allows retrieving authentication tokens and communicate with the Open Peer Power websocket API. It can be used to integrate Open Peer Power into your apps. It has 0 dependencies.

## Trying it out

Check [the demo](https://opp-auth-demo.glitch.me/). The repository also includes an [example client](https://github.com/openpeerpower/openpeerpower-js-websocket/blob/main/example.html):

Clone this repository, then go to openpeerpower-js-websocket folder and run the following commands:
...
To initialize a connection, you need an authentication token for the instance that you want to connect to. This library implements the necessary steps to guide the user to authenticate your website with their Open Peer Power instance and give you a token. All you need from the user is the url of their instance.
...
```js
getAuth({ oppUrl: "http://localhost:8123" });
```

| Option      | Description                                                                                                                                                                                               |
| ----------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| oppUrl      | The url where the Open Peer Power instance can be reached. This option is needed so we know where to redirect the user for authentication. Once redirected back, it is not needed to pass this option in. |
| clientId    | Client ID to use. Client IDs for Open Peer Power is the url of your application. Defaults to domain of current page. Pass `null` if you are making requests on behalf of a system user.                   |
...
You can subscribe to the entities of Open Peer Power. Your callback will be called when the entities are first loaded and on every change to the state of any of the entities after that. The callback will be called with a single object that contains the entities keyed by entity_id.

The function `subscribeEntities` will return an unsubscribe function.

```javascript
import { subscribeEntities } from "openpeerpower-js-websocket";
...
```javascript
import { entitiesColl } from "openpeerpower-js-websocket";
...
You can subscribe to the config of Open Peer Power. Config can change when either a component gets loaded.

The function `subscribeConfig` will return an unsubscribe function.

```javascript
import { subscribeConfig } from "openpeerpower-js-websocket";
...
```javascript
import { configColl } from "openpeerpower-js-websocket";
...
You can subscribe to the available services of Open Peer Power. Services can change when a new service gets registered or removed.

The function `subscribeServices` will return an unsubscribe function.

```javascript
import { subscribeServices } from "openpeerpower-js-websocket";
...
```javascript
import { servicesColl } from "openpeerpower-js-websocket";
...
```javascript
import { getCollection } from "openpeerpower-js-websocket";
...
String containing the current version of Open Peer Power. Examples:
...
Subscribe to all or specific events on the Open Peer Power bus. Calls `eventCallback` for each event that gets received.
...
Call an endpoint in Open Peer Power that creates a subscription. Calls `callback` for each item that gets received.
...
- `getUser(connection) -> Promise<OppUser>`
- `callService(connection, domain, service, serviceData?, target?) -> Promise` (Support for `target` was added in Open Peer Power Core 2021.3)

The following are also available, but it's recommended that you use the subscribe methods documented above.

- `getStates(connection) -> Promise<OppEntity[]>`
- `getServices(connection) -> Promise<OppEntity[]>`
- `getConfig(connection) -> Promise<OppEntity[]>`
...
Look at https://github.com/keesschollaart81/vscode-open-peer-power/blob/main/src/language-service/src/openpeerpower/socket.ts as an example using ws.
...
createConnection({
  createSocket() {
    ...
    // TODO: Handle authentication with Open Peer Power yourself :)
